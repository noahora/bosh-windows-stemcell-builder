...
**2)** If you would like to simply sysprep the image:

  - Run the following powershell command `Invoke-Sysprep -IaaS vsphere -ApplyLgpo $False -NewPassword <NEW_PASSWORD> -ProductKey <PRODUCT_KEY> -Owner <OWNER> -Organization <ORGANIZATION>`
  - This will power off the VM
  - Do not turn the VM back on before exporting
